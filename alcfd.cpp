...
#include "alcfd.h"
#include "srcs/CFDMeasure.h"
#include "srcs/CFDMeasure_2.h"

using std::string;

enum dataset { abalone, adult, soccer, sp500 };

enum error_percent { zero_one, zero_five, one, two };
...
void al_cfd::assess(const std::vector<std::string>& lhs, const std::string& rhs) {
  ...
  init_sampling::execute(lst_tuple_pair, vec_predicates, init_combination, plis, pliRecords, attributes, clean_records,
                         map_pred_pli);
...
void al_cfd::execute() {
  ...
  for (unsigned epoch = 0; epoch != EPOCH; ++epoch) {
    if (epoch > 10)
      SUPPORT_COUNT = BIG_MIN_SUPPORT_COUT;
    else
      SUPPORT_COUNT = SMALL_MIN_SUPPORT_COUNT;
    ...
    if (no_adjustment <= WAIT_ROUND) resampling_time += static_cast<int>(t5 - t5_before) / CLOCKS_PER_MSEC;
    ...
    if (no_adjustment <= WAIT_ROUND) refinement_time += static_cast<int>(t6 - t5) / CLOCKS_PER_MSEC;
  }
  ...
  out_result << std::endl
             << "rounds: " << round_count << ", resampling_time: " << resampling_time
...
void al_cfd::init_sampling() {
  ...
  for (const auto& tp : lst_tuple_pair) {
    lst_tuple_pairs.emplace_back(std::make_shared<TuplePair>(records[*tp.begin()], records[*tp.rbegin()]));
  }
...
void al_cfd::init_learning() {
  ...
  for (const auto& tp : *all_lst_tp_label_tf) {
    tf_ff_i_ii_tp.emplace_back(tp);
  }
  for (const auto& tp : *all_lst_tp_label_ffi) {
    tf_ff_i_ii_tp.emplace_back(tp);
  }
  for (const auto& tp : *all_lst_tp_label_ffii) {
    tf_ff_i_ii_tp.emplace_back(tp);
  }
...
VecUint find_certain_value_set_auto(unsigned attr_id, std::vector<Pli>& plis, const VecSpRec& records,
...
void backtrack_auto(VecVecUint& res, const VecVecUint& init_combination, const VecSpRec& records, const VecUint& nums,
                    unsigned attr_id, unsigned k, int first, VecUint& curr, const VecUpVecUint& rhs_pli) {
  if (curr.size() == k) {
...
void combinations_auto(VecVecUint& res, const VecVecUint& init_combination, const VecSpRec& records,
                       const VecUint& nums, unsigned attr_id, unsigned k, const VecUpVecUint& rhs_pli) {
  VecUint curr;
...
bool al_cfd::auto_ajustment(unsigned epoch) {
  ...
  for (auto it = rule_repository_top_k.begin(); it != rule_repository_top_k.end(); ++rule_index) {
    ...
    for (int i = 0; i != lhs.size(); ++i) {
        ...
        if (i == 0) {
          ...
          for (const VecUint& now_vec : last_lst_tuple_pair) {
            ...
            for (const auto& pair : new_map) {
              if (pair.second.size() >= 2) now_lst_tuple_pair.emplace_back(pair.second);
            }
          }
      ...
      // CC = 01
        ...
        if (i == 0) {
          ...
          if (!cluster.empty()) last_lst_tuple_pair.emplace_back(cluster);
        } else {
          now_lst_tuple_pair.clear();
          for (const VecUint& now_vec : last_lst_tuple_pair) {
            std::unordered_map<unsigned, VecUint> new_map;
    ...
    if (predicates_space.at(RHS).type == 0) {
      ...
      if (!cluster.empty()) rhs_pli.emplace_back(std::make_unique<VecUint>(cluster));
    }
    for (const auto& cluster : now_lst_tuple_pair) {
      ...
      for (const auto& tp_id : ans) {
        ...
        if (!isTrash) {
          ...
          if (rule.get_information() == "(education=Some-college,relationship=Husband,)=>sex=Male") std::cout << "";
          if (rule.get_information() == "(relationship=Husband,income=MoreThan50K,)=>sex=Male") std::cout << "";
          if (rule.get_information() ==
              "(age=18-21,education=HS-grad,maritalstatus=Never-married,)=>income=LessThan50K")
            std::cout << "";
          if (rule.get_information() == "(age=18-21,education=HS-grad,)=>income=LessThan50K") std::cout << "";
    ...
    if (predicates_space.at(RHS).type == 1) {
      if (std::all_of(lhs.begin(), lhs.end(),
                      [&](unsigned pred_id) { return predicates_space.at(pred_id).type == 1; })) {
        for (const auto& pair : map_id_record) {
          ...
          if (std::find(error_attr_id.begin(), error_attr_id.end(), rhs_attr_id) == error_attr_id.end() &&
            ...
            if (std::all_of(lhs.begin(), lhs.end(), [&](unsigned pred_id) {
              ...
              append_predicate_into_rule(*pair.second, rule, pair.second->get_feature(), hasAppended, epoch);
              if (!isTrash) {
                cant_adjustment = false;
  ...
  if (epoch == EPOCH - 1) {
    for (auto&& rrule : learned_rules)
      if (!in_trash_repository_set(rrule)) add_most_general_rule(rrule);
  }
...
VecUint find_certain_value_set_main(unsigned attr_id, std::vector<Pli>& plis, const VecSpRec& records,
                                    const std::string& value) {
  VecUint ans;
...
void al_cfd::compute_recall() {
  ...
  for (int record_index = 0; record_index != records.size(); ++record_index) {
    ...
    int now_attr_id = 0;
    for (auto it_clean = sp_rec_clean->begin(), it_dirty = sp_rec_dirty->begin(); it_clean != sp_rec_clean->end();
         ++it_clean, ++it_dirty, ++now_attr_id) {
      if (*it_clean != *it_dirty) {
        ...
        for (const auto& rule : origin_rules) {
          ...
          std::sort(lhs.begin(), lhs.end(),
                    [&](unsigned lhs_pred_id, unsigned rhs_pred_id) { return lhs_pred_id < rhs_pred_id; });
        ...
        for (const auto& rule : rule_repository_top_k) {
          ...
          if (can_detect) {
            ++find_dirty;
            if (is_origin_find) ++my_in_origin_find_dirty;
            break;
          }
        }
      }
  ...
  if (in_file.is_open()) {
    ...
    while (std::getline(in_file, str)) {
      if (str.find("record:", 0, 1) != std::string::npos) {
        ...
        auto colon = str.find(':');
        if (str.find('T', colon))
          map_id_record[id]->set_label(true);
        else if (str.find('F', colon))
          map_id_record[id]->set_label(false);
      }
    }
    in_file.close();
  } else {
    ...
    if (have_no_variable) {
      ...
      for (const auto& pair : map_id_record) {
        ...
        if (match_lhs) {
          unsigned RHS = rule.get_rhs();
          auto attr_id = predicates_space.at(RHS).attr_id;
          if (std::find(error_id.begin(), error_id.end(), attr_id) != error_id.end()) continue;
...
bool al_cfd::verify_by_true(const Rule& rule) {
  ...
  if (in_trash_repository_set(rule)) return false;
  auto lhs = rule.get_lhs();
  auto RHS = rule.get_rhs();
  ...
  if (predicates_space.at(rule.get_rhs()).type == 1) {
    ...
    if (std::all_of(lhs.begin(), lhs.end(), [&](unsigned pred_id) { return predicates_space.at(pred_id).type == 1; })) {
      for (const auto& pair : map_id_record) {
...
bool al_cfd::support_by_n_true(Rule& rule, unsigned epoch) {
  ...
  if (epoch < 10)
    ...
    return support > BIG_MIN_SUPPORT_COUT;  // && error_supp > BIG_MIN_SUPPORT_COUT;
    ...
    if (!in_trash_repository_set(rule)) {
      ...
      for (int i = 1; i != lhs.size(); ++i) {
        ...
        for (const auto& new_lhs : new_lst_lhs) {
          ...
          if (new_rule.get_information() == "(f,)=>g") std::cout << "";
...
void al_cfd::add_most_general_rule(Rule& rule) {
  bool most_general =
      add_most_general_rule(rule, rule_repository) && add_most_general_rule(rule, rule_repository_top_k);
...
bool al_cfd::is_same_rule(const Rule& rule1, const Rule& rule2) {
  ...
  if (rule1.get_rhs() == rule2.get_rhs()) {
    if (std::all_of(lhs1.begin(), lhs1.end(),
                    [&lhs2](unsigned pred_id) { return std::find(lhs2.begin(), lhs2.end(), pred_id) != lhs2.end(); }) &&
        std::all_of(lhs2.begin(), lhs2.end(),
                    [&lhs1](unsigned pred_id) { return std::find(lhs1.begin(), lhs1.end(), pred_id) != lhs1.end(); })) {
      return true;
    }
  }
  return false;
...
int al_cfd::compute_support(Rule& rule) {
  ...
  if (predicates_space.at(RHS).type == 1) {
    if (std::all_of(lhs.begin(), lhs.end(),
                    [&](const unsigned& pred_id) { return predicates_space.at(pred_id).type == 1; })) {
      is_pure_cfd = true;
    }
  }
...
void al_cfd::predict(unsigned epoch) {
  ...
  std::sort(re_globle_lst_tuple_pairs.begin(), re_globle_lst_tuple_pairs.end(),
            [](const SpTp& lhs, const SpTp& rhs) { return lhs->ViolateCFDsCount() > rhs->ViolateCFDsCount(); });
}
...
void al_cfd::re_sampling_global() {
  ...
  for (const auto& lst_tuple_pair : lst_lst_tuple_pair) {
    for (const auto& tp : lst_tuple_pair) {
      re_globle_lst_tuple_pairs.emplace_back(std::make_shared<TuplePair>(records[*tp.begin()], records[*tp.rbegin()]));
    }
  }
  for (auto& tp : re_globle_lst_tuple_pairs) {
    tp->set_feature(predicates_space);
  ...
  for (const SpTp& sptr_tp : re_globle_lst_tuple_pairs) {
    if (sptr_tp->get_relative_pred().empty() && sptr_tp->get_non_pred().empty()) set_relative_predicates(*sptr_tp);
  }
}

void al_cfd::re_sampling_predicted(unsigned epoch) {
  if (epoch == 14) std::cout << "";
...
void al_cfd::print_rule_repository(const std::string& filename) {
  ...
  for (const auto& rule : rule_repository_top_k) {
    print_str += rule.get_information() + " | whole_supp: " + std::to_string(rule.get_whole_supp()) +
                 " | whole_vio: " + std::to_string(rule.get_whole_violation()) + " | whole_conf: " +
                 std::to_string(rule.get_whole_conf())
...
void al_cfd::append_predicate_into_rule(const T& tp, const Rule& rule, std::vector<bool> feature,
  ...
  for (unsigned i = 0; i != feature.size(); ++i) {
    if (constant_attr_id.find(predicates_space.at(i).attr_id) != constant_attr_id.end()) continue;
...
void al_cfd::compute_support() {
  ...
  for (Rule& rule : rule_repository) {
    if (rule.get_whole_supp() == -1 || rule.get_whole_violation() == -1) compute_whole_vio(rule);
  }
  ...
  std::sort(rule_repository.begin(), rule_repository.end(), [&](const Rule& rule1, const Rule& rule2) {
    ...
    // return
    if (lhs1.size() != lhs2.size()) return lhs1.size() < lhs2.size();
    ...
    if (error_attributes.find(predicates_space.at(rule1.get_rhs()).attr_id) != error_attributes.end()) error_count_1++;
    if (error_attributes.find(predicates_space.at(rule2.get_rhs()).attr_id) != error_attributes.end()) error_count_2++;
    ans1 = error_count_1 / (lhs1.size() + 1.0);
    ans2 = error_count_2 / (lhs2.size() + 1.0);
    // return
    if (fabs(ans1 - ans2) > 0.001) return ans1 > ans2;
    if (rule1.get_support() != rule2.get_support()) return rule1.get_support() > rule2.get_support();
  ...
  for (unsigned i = 0; i != rule_repository.size(); ++i) {
    if (deleted_index.find(i) != deleted_index.end()) {
      if (rule_repository[i].get_information() == "(age=18-21,education=HS-grad,)=>income=LessThan50K") std::cout << "";
      if (rule_repository[i].get_information() == "(birthplace,city,season=2013,)=>position") std::cout << "";
  ...
  if (rule_repository.size() >= RULE_REPOSITORY_K) {
    ...
    for (unsigned i = 0; i != rule_repository.size(); ++i) {
      ...
      if (j == rule_repository.size()) {
        if (i < rule_repository.size()) rule_repository.resize(i);
        break;
      }
...
void al_cfd::compute_whole_support() {
  ...
  std::sort(rule_repository.begin(), rule_repository.end(), [&](const Rule& rule1, const Rule& rule2) {
    ...
    // return
    if (fabs(ans1 - ans2) > 0.001) return ans1 > ans2;
    if (rule1.get_whole_conf() != rule2.get_whole_conf()) return rule1.get_whole_conf() > rule2.get_whole_conf();
    if (rule1.get_support() != rule2.get_support()) return rule1.get_support() > rule2.get_support();
    if (rule1.get_error_support() != rule2.get_error_support())
      return rule1.get_error_support() > rule2.get_error_support();
    return true;
  ...
  for (unsigned i = 0; i != rule_repository.size(); ++i) {
    auto conf = rule_repository[i].get_whole_conf();
    auto rhs = rule_repository[i].get_rhs();
    if ((conf != -1 && conf < 0.9) || (freq_rhs.find(rhs) == freq_rhs.end() && (i % 2 == 0 || i % 3 == 0))) {
      deleted_index.insert(i);
    }
  }
  ...
  deleted_index.clear();
  for (unsigned i = 0; i != rule_repository.size(); ++i) {
    if (deleted_index.find(i) != deleted_index.end()) continue;
    ...
    for (unsigned j = i + 1; j != rule_repository.size(); ++j) {
      if (deleted_index.find(j) != deleted_index.end()) continue;
...
void al_cfd::compute_whole_support(Rule& rule) {
  int violation =
      CFDMeasure_2::compute_violation(clean_plis, clean_pliRecords, attributes, clean_records, rule, predicates_space);
  rule.set_whole_violation(violation);
  int support =
      CFDMeasure_2::compute_support(clean_plis, clean_pliRecords, attributes, clean_records, rule, predicates_space);
  rule.set_whole_supp(support);
