...
VecUint find_certain_value_set(unsigned attr_id, std::vector<Pli>& plis, const VecSpRec& records,
...
void ReSampling::backtrack(VecVecUint& res, const VecVecUint& init_combination, const VecSpRec& records,
                           const VecUint& nums, unsigned k, int first, VecUint& curr, const VecUpVecUint& rhs_pli) {
  // this is a change
  ...
  if (curr.size() == k) {
    ...
    if (this_used_tuple_pairs.find(tp_string) != this_used_tuple_pairs.end()) return;
...
void ReSampling::re_sampling_tuple_pair_bfs(VecVecVecUint& lst_tuple_pair, const VecVecUint& init_combination,
                                            const VecStr& attributes, const VecSpRec& records,
                                            const std::vector<Rule>& rule_repository,
                                            const PredicatesSpace& predicates_space,
                                            const std::unordered_set<std::string>& used_tuple_pairs) {
  ...
  for (const Rule& rule : rule_repository) {
    // rule: CC=01, AC => CT
    std::vector<unsigned> lhs = rule.get_lhs();
    std::sort(lhs.begin(), lhs.end(), [&](unsigned id_1, unsigned id_2) { return id_1 < id_2; });
    unsigned i = 0, key = 0;
        ...
        if (i == 0) {
          ...
          for (const auto& cluster : *pli) {
            for (const auto& cluster_ : *now_pli) {
              ...
              if (ans.size() >= 2) ans_set->emplace_back(std::make_unique<VecUint>(ans));
            }
          }
          pli = ans_set;
          key_pli[key] = pli;
    ...
    for (const auto& cluster : *pli) {
      ...
      if (!ans.empty()) lst_tuple_pair.emplace_back(ans);
    }
    clock_t t3 = clock();
...
void ReSampling::re_sampling_tuple_pair_pliRecords(VecVecVecUint& lst_tuple_pair, const VecVecUint& init_combination,
  ...
  for (Rule& rule : rule_repository) {
    ...
    if (rule.get_whole_violation() == -1) rule.set_whole_violation(violation);
...
void ReSampling::build_plis(const VecStr& attributes, const VecSpRec& records) {
  ...
  for (auto&& record : records) {
    for (auto&& str : record->get_data()) {
      bytes += str.size();
    }
  }
  std::cout << "records use " << bytes << " bytes" << std::endl;

  bytes = 0;
  for (auto&& line : *pliRecords) {
    for (auto&& item : *line) {
      bytes += sizeof(item);
    }
  }
  std::cout << "pliRecords use " << bytes << " bytes" << std::endl;
  ...
  std::cout << "pli use " << bytes << " bytes" << std::endl;
}
