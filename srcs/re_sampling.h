...
class ReSampling {
 public:
  ...
  void re_sampling_tuple_pair_pliRecords(VecVecVecUint& lst_tuple_pair, const VecVecUint& init_combination,
                                         const VecStr& attributes, const VecSpRec& records,
                                         std::vector<Rule>& rule_repository, const PredicatesSpace& predicates_space,
                                         const std::unordered_set<std::string>& used_tuple_pairs,
                                         std::unordered_map<std::string, SpVecVecUint>& map_pred_pli);

  void re_sampling_tuple_pair_dfs(VecVecVecUint& lst_tuple_pair, const VecVecUint& init_combination,
                                  const VecStr& attributes, const VecSpRec& records,
                                  const std::vector<Rule>& rule_repository, const PredicatesSpace& predicates_space,
                                  const std::unordered_set<std::string>& used_tuple_pairs);

  void re_sampling_tuple_pair_bfs(VecVecVecUint& lst_tuple_pair, const VecVecUint& init_combination,
                                  const VecStr& attributes, const VecSpRec& records,
                                  const std::vector<Rule>& rule_repository, const PredicatesSpace& predicates_space,
                                  const std::unordered_set<std::string>& used_tuple_pairs);
