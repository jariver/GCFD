...
void init_sampling::execute(VecVecUint& lst_tuple_pair, VecPre& vec_predicates, VecVecUint& combination,
                            std::vector<Pli>& plis, UpVecUpVecInt& pliRecords, const VecStr& attributes,
                            const VecSpRec& records, std::unordered_map<std::string, SpVecVecUint>& map_pred_pli) {
...
void init_sampling::preprocess(std::vector<Pli>& plis, const VecStr& attributes, const VecSpRec& records) {
  clock_t t0 = clock();
...
void init_sampling::sampling_tuple_pairs_pliRecords(VecVecUint& lst_tuple_pair, VecVecUint& init_combination,
                                                    std::vector<Pli>& plis, UpVecUpVecInt& pliRecords,
  ...
  std::sort(plis.begin(), plis.end(),
            [](const Pli& lhs, const Pli& rhs) { return lhs.partition->size() > rhs.partition->size(); });
  ...
  std::sort(plis.begin(), plis.end(), [](const Pli& lhs, const Pli& rhs) { return lhs.attr < rhs.attr; });
  ...
  for (auto& com_attrs : combination) {
    ...
    // BEGIN-1
    std::sort(com_attrs.begin(), com_attrs.end(),
              [&](unsigned lhs_attr_id, unsigned rhs_attr_id) { return lhs_attr_id < rhs_attr_id; });
...
void init_sampling::create_frequency_items(VecPre& vec_predicates, const std::vector<Pli>& plis,
                                           const VecStr& attributes, const VecSpRec& records) {
  for (const auto& pli : plis) {
    for (const auto& cluster : *pli.partition) {
      ...
      if (cluster->size() >= records.size() * MIN_WHOLE_SUPPORT_RATE) {
        vec_predicates.emplace_back(1, attributes[pli.attr], pli.attr, records[cluster->at(0)]->at(pli.attr));
      }
    }
  }
}

void init_sampling::create_pliRecords(UpVecUpVecInt& pliRecords, const std::vector<Pli>& plis, const VecStr& attributes,
