...
class PredicatesSpace {
 public:
  ...
  Predicate at(const unsigned i) const { return predicates->at(i); }

  auto begin() { return predicates->begin(); }

  auto end() { return predicates->end(); }

  auto begin() const { return predicates->cbegin(); }

  auto end() const { return predicates->cend(); }

  unsigned size() const { return predicates->size(); }

  void clear() const { predicates->clear(); }

  void emplace_back(Predicate&& predicate) { predicates->emplace_back(std::move(predicate)); }
