...
void TuplePair::set_feature(const PredicatesSpace& predicatesSpace) {
  for (const Predicate& predicate : predicatesSpace) {
    if (left_data->at(predicate.attr_id) == right_data->at(predicate.attr_id)) {
      ...
      else if (left_data->at(predicate.attr_id) == predicate.value)
        feature.emplace_back(1);
      else
...
void TuplePair::emplace_back_relative_pred_2(unsigned index) { relative_predicates_2.emplace_back(index); }

void TuplePair::emplace_back_non_pred(unsigned index) { non_relative_predicates.emplace_back(index); }

void TuplePair::emplace_back_relative_pred(unsigned index) { relative_predicates.emplace_back(index); }
